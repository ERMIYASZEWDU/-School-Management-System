...
export const ParentChildGrades = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const [gradesData, childData] = await Promise.all([
              ...
              <tbody>
                {grades.length > 0 ? (
                  grades.map((grade) => {
                    ...
                    return (
                      <tr key={grade._id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-blue-50 dark:hover:bg-blue-900/40">
                        ...
                        <td className="p-4 text-blue-600 dark:text-blue-400 text-sm">{grade.teacherId?.name || 'N/A'}</td>
